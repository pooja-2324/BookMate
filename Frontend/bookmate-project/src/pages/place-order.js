          ...
          <>
            ...
            <div className="space-y-4">
              {cartData?.map((ele) => (
                <div
                  key={ele._id}
                  className="bg-white shadow-md rounded-lg p-4 flex gap-4 items-center border border-[#3D405B]"
                >
                  {/* Book Cover Image */}
                  <img
                    className="w-32 h-40 object-cover rounded-md shadow-sm"
                    src={ele.book?.coverImage}
                    alt={ele.book?.modifiedTitle}
                  />
                  ...
                  <div className="flex-1">
                    ...
                    {ele.rent && (
                      <>
                        ...
                        <h3 className="text-[#3D405B]">Caution Deposit:{ele.rent.pricing.cautionDeposit}</h3>
                        <h3 className="text-[#3D405B]">deliveryFee:{ele.rent.pricing.deliveryFee}</h3>
                        <h3 className="text-[#3D405B]">Platform Fee:{ ele.rent.pricing.platformFee}</h3>
                        <h3 className="text-[#3D405B]">Reading Fee:{ ele.rent.pricing.readingFee}</h3>
                    ...
                    {ele.buy && (
                      <>
                        <h3 className="text-[#3D405B]">
                          Sale Price: ₹{ele.buy.sellPrice}
                        </h3>
                        
                        <h3 className="text-[#3D405B]">Platform Fee:{ele.buy.platformFee}</h3>
                        <h3 className="text-[#3D405B]">Delivery Fee:{ele.buy.deliveryFee}</h3>
                        <h3 className="text-[#1A1A1A] font-semibold">
                          Total Amount: ₹
                          {ele.buy.sellPrice + ele.buy.platformFee + ele.buy.deliveryFee}
                        </h3>
                      </>
                    )}
                  </div>
            ...
            {/* Total Amount */}
            <h4 className="text-2xl font-bold text-[#1A1A1A] text-center mt-6">
              Total: ₹{totalAmount}
            </h4>
            ...
            <div className="flex justify-center mt-6">
              <button
                onClick={handleOrder}
                className="bg-[#2C3E50] text-white px-8 py-3 rounded-lg font-semibold hover:bg-[#3D405B] transition-colors"
              >
                Pay ₹{totalAmount}
              </button>
            </div>
